...
const categoryController = {
    ...
    deleteCategory: async (req, res) => {
        try {
            ...
            const productsUsingCategory = await Product.find({ categoryId: categoryId });

            if (productsUsingCategory.length > 0) {
                return res.status(400).json({ message: 'Không thể xóa danh mục vì có sản phẩm đang sử dụng nó.' });
            }
